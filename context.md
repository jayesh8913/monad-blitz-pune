...
* **MetaMask Integration**: Connects dynamically to Monad Testnet (`Chain ID: 10143`), switches networks automatically, and tracks real-time user balances for native `MON` and mock `ETH` faucet tokens.
* **Control Panels**:
  * **Sentiment Trading**: Captures YouTube URLs, feeds them to the backend parser, displays loader timeline metrics, and logs results. Supports manual deposit/withdraw operations and manual swaps.
...
* **DEX & NFT Integrations**: Autonomously performs `MockETH -> MON` swaps (on BULLISH sentiment) or `MON -> MockETH` swaps (on BEARISH sentiment). Minting a commemorative ERC721 trade badge NFT is triggered on successful trades.

### 3. ⛓️ Smart Contracts (`packages/contracts`)
* **MockETH.sol**: Standard ERC20 token containing `mintFaucet(uint256 amount)` to distribute test funds.
* **MockDEX.sol**: Swaps native `MON` and `MockETH` to emulate automated DeFi narrative execution.
...
MOCK_ETH_ADDRESS=0x5FbDB2315678afecb367f032d93F642f64180aa3
